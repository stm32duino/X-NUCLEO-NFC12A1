/**
 ******************************************************************************
 * @file    X_NUCLEO_NFC12A1_HelloWorld.ino
 * @author  AST
 * @version V1.0.0
 * @date    2 September 2021
 * @brief   Arduino test application for the STMicrolectronics X-NUCLEO-NFC12A1
...
RfalRfST25R500Class rfst25r500(&dev_spi, CS_PIN, IRQ_PIN, ST25R_RESET);
RfalNfcClass rfal_nfc(&rfst25r500);
...
static void checkUserButton(void)
{
  ...
  if (digitalRead(USER_BTN) == PushButtonState) {
    ...
    if (ndefDemoFeature != NDEF_DEMO_READ) {
      timer = rfst25r500.timerCalculateTimer(NDEF_WRITE_FORMAT_TIMEOUT);
      timerLed = rfst25r500.timerCalculateTimer(NDEF_LED_BLINK_DURATION);
    }
  }
}
...
void setup()
{
  ...
  Serial.println("Welcome to X-NUCLEO-NFC12A1");
...
void loop()
{
  ...
  if ((ndefDemoFeature != NDEF_DEMO_READ) && (rfst25r500.timerIsExpired(timer))) {
  ...
  if (ndefDemoFeature != NDEF_DEMO_READ) {
    if (rfst25r500.timerIsExpired(timerLed)) {
      timerLed = rfst25r500.timerCalculateTimer(NDEF_LED_BLINK_DURATION);
      ledOn = !ledOn;
    }
    if (ledOn) {
